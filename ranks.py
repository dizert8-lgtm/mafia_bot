...
🤵 Правая рука    (underboss)   — заместитель
💼 Капо           (capo)        — старший в группе
🗡️ Мафиозо       (mafioso)     — проверенный член
🃏 Associate      (associate)   — кандидат
"""
...
def get_clan_members_by_rank(clan_id: int) -> list:
    ...
    rows = c.fetchall()
    conn.close()
    # Сортируем по порядку иерархии
    return sorted(rows, key=lambda x: RANK_ORDER.index(x[1]) if x[1] in RANK_ORDER else 99)