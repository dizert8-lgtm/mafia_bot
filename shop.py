...
WEAPONS = [
    ...
    {"id":"w13", "name":"Золотой пистолет",      "price":100000,   "desc":"Позолоченный пистолет ручной работы. Статус и смерть в одном.",               "photo":"https://i.ibb.co/5gFnhG8d/photo-4-2026-05-14-20-24-58.jpg"},
...
CARS = [
    ...
    {"id":"c14", "name":"Золотой Rolls-Royce",    "price":1000000,  "desc":"Покрыт золотом 24 карата. Один в мире. Принадлежит только боссу.",            "photo":"https://i.ibb.co/B5g5cvtg/photo-2026-05-14-20-49-54.jpg"},
]
...
def get_inventory_text(user_id: int) -> str:
    """Возвращает текст коллекции для профиля."""
    owned = get_owned_items(user_id)
    if not owned:
        return ""
    total_value = sum(ALL_ITEMS[i]["price"] for i in owned if i in ALL_ITEMS)
    return f"\n💎  <b>Коллекция:</b>  {len(owned)} предм.  ({total_value:,} монет)"
...
async def _show_category(query, user_id: int, cat_key: str, page: int = 0):
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀", callback_data=f"shop_page_{cat_key}_{page-1}"))
...
async def _show_item(query, user_id: int, item_id: str):
    ...
    status   = "✓ В коллекции" if is_owned else f"💰 Цена: {item['price']:,}"
    text     = (
        f"<b>{item['name']}</b>\n{'─'*22}\n\n"
        f"{item['desc']}\n\n"
        f"{status}\n"
...
async def _show_inventory(query, user_id: int):
    owned = get_owned_items(user_id)
...
async def shop_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "shop_back":
        ...
        try:
            await query.edit_message_caption(
                caption=f"<b>[ Магазин семьи ]</b>\n{'─'*22}\n\n💰 Монеты: <b>{coins:,}</b>\n🎒 Предметов: <b>{len(owned)}</b>\n\n<i>Выбери категорию:</i>",
                reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="HTML")
        except: pass
        ...
        await _show_inventory(query, user_id)
